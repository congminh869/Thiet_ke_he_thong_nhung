...
// WiFi credentials
const char ssid[] = "TP-LINK_8149B2";
const char password[] =  "sontungmtp";

// Server, file, and port
const char hostname[] = "192.168.2.105";
const String uri = "/";
const int port = 1337;
...
void loop() {
  ...
  while (i < num_samples) {
    if (millis() >= timestamp + (1000 / sample_rate)) {
      ...
      json_x.add(event.acceleration.x);
      json_y.add(event.acceleration.y);
      json_z.add(event.acceleration.z);
